            ...
            <div class="row justify-content-center mt-2">
                <div class="col-10">
                    <table class="table" id="tableCart">
                        ...
                        <c:if test="${!(sessionScope.carts == null || sessionScope.carts.size()==0)}">
                            <h1>List carts</h1>
                            <h3>Total amount: ${df.format(requestScope.totalmoney)}</h3>  
                        </c:if>
                        <c:forEach items="${carts}" var="c">
                            <form action="update-quantity">
                                ...
                                <td>Price: ${c.value.product.price}</td>
                                <td>So luong hang trong kho: ${c.value.product.quantity}</td>
                                <td>Quantity :<input type="number" name="quantity" onchange="this.form.submit()" value="${c.value.quantity}"  min="1" max="${c.value.product.quantity}" onkeydown="return false" onpaste="return false" required
                                                     style="width: 50px;
                                                     height: 20px;
                                                     font-size: 14px;
                                                     padding: 5px;
                                ...
                                <td class="text-center"><a href="deletecart?categoryID=${c.key}" class="btn btn-light bi bi-x"></button></td>
                                </tr>
                            </form>
                        </c:forEach>
